...
doc.add_paragraph()
p=doc.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER
sf(p.add_run('ERP 개발 요청용  |  2026-05  |  R4'),size=10,color=(0x5A,0x60,0x70))
doc.add_page_break()
...
h2('7-5. 구현 상태 (v10 기준)')
t=tbl(['항목','v10 상태','비고'],[
    ['Invoice No. 추출','✅ 구현','저장 시 기록'],
    ['Invoice Date / AWB No. 추출','✅ 구현','참고용 표시'],
    ['MPW Chip 블록 탐지 (AL- 패턴)','✅ 구현','두 자리 번호·페이지 브레이크 대응'],
    ['과제명 / TM Code / PO No. 추출','✅ 구현','—'],
    ['Qty / 단위 추출','✅ 구현','MPW: EA, Wafer: PCS'],
    ['Lot ID + Wafer No. 분리','✅ 구현','# 기준 분리 처리'],
    ['Wafer 타입 Invoice 파싱','✅ 구현','* 기준 Lot ID / Qty 추출, Wafer No. 범위 파싱'],
    ['Wafer No. 범위 자동 분배 적용','✅ 구현','# 있으면 자동 채움, 없으면 수기 입력'],
    ['GRAND TOTAL 표시','✅ 구현','파싱 합계 + Carton 합계 각각 별도 표시'],
],col_widths=[5.5,2,8.5])
shade_row(t,1,'F3F9ED'); shade_row(t,2,'F3F9ED'); shade_row(t,3,'F3F9ED')
shade_row(t,4,'F3F9ED'); shade_row(t,5,'F3F9ED'); shade_row(t,6,'F3F9ED')
shade_row(t,7,'F3F9ED'); shade_row(t,8,'F3F9ED'); shade_row(t,9,'F3F9ED')
...
out='/home/user/mj-automation-dashboard/docs/입출하관리_기능명세서_R4.docx'
doc.save(out)
print('saved:', out)
